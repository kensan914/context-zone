 ...
 */
import {
    tryCatch, props, setProp, _global,
    getPropertyDescriptor, getArrayOf, extend
...
//
const task = { awaits: 0, echoes: 0, id: 0 }; // ゾーンエコー使用時の進行中のmacro task。The ongoing macro-task when using zone-echoing.
// echoes !== 0: we are in zone-echoing mode!
var taskCounter = 0; // ID counter for macro tasks.
var zoneStack = []; // Stack of left zones to restore asynchronically.
var zoneEchoes = 0; // zoneEchoesは、native await式間のゾーンを持続させるためには必須です。zoneEchoes is a must in order to persist zones between native await expressions.
...
export function newScope(fn, props, a1, a2) {
    ...
    // プロミスパッチの準備（usePSDで行います）。 Prepare for promise patching (done in usePSD):
    var globalEnv = globalPSD.env;
    psd.env = patchGlobalPromise ? { // patchGlobalPromise = true
        Promise: DexiePromise, // IDB+Promiseが活躍するChromeやEdgeでは、window.Promiseの変更は省略できるかもしれません。 Changing window.Promise could be omitted for Chrome and Edge, where IDB+Promise plays well!
    ...
    console.log("psd.ref", psd.ref);
    if (props) extend(psd, props);

    // unhandledsとonunhandledは、ここでは特に設定する必要はありません。 unhandleds and onunhandled should not be specifically set here.
    // 親プロトタイプのままにしておきます。 Leave them on parent prototype.
    // unhandleds.push(err) は親のプロトタイプにプッシュします。unhandleds.push(err) will push to parent's prototype
    // onunhandled() は親の onunhandled を呼び出します (ただし、このスコープの this-pointer を使用します)。 onunhandled() will call parents onunhandled (with this scope's this-pointer though!)
    ++parent.ref;
...
// scopeFunc が NativePromise を返した場合に呼び出す関数。Function to call if scopeFunc returns NativePromise
// Promise.all() の引数に含まれる各 NativePromise についても同様です。Also for each NativePromise in the arguments to Promise.all()
...
function switchToZone(targetZone, bEnteringZone) {
    ...
    PSD = targetZone; // 実際のゾーン切り替えはこの行で発生します。 The actual zone switch occurs at this line.

    // globalゾーンから離れる際に毎回スナップショットします。 Snapshot on every leave from global zone.
    ...
    if (patchGlobalPromise) {
        ...
        // envを切り替えます（PSDゾーンまたはグローバルゾーンのいずれかである可能性があります)。Swich environments (may be PSD-zone or the global zone. Both apply.)
        var targetEnv = targetZone.env;

        // ネイティブとグローバルのPromise用にPromise.prototype.thenを変更します(ポリフィル環境では異なりますが、両方にアクセスできます。)。Change Promise.prototype.then for native and global Promise (they MAY differ on polyfilled environments, but both can be accessed)
        // パッチ適用されたメソッドのクロージャにtargetZoneが含まれているため、ゾーンの変更ごとに行う必要があります。Must be done on each zone change because the patched method contains targetZone in its closure.
        nativePromiseProto.then = targetEnv.nthen;
        GlobalPromise.prototype.then = targetEnv.gthen;

        if (currentZone.global || targetZone.global) {
            // グローバルゾーンからの離脱、またはグローバルゾーンへの進入。グローバルプロミスのパッチ/復元の時間です。Leaving or entering global zone. It's time to patch / restore global Promise.

            // このPromiseをwindow.Promiseに設定することで、遷移した非同期関数がFirefox、Safari、IE、そしてZonejsやangularでも動作するようにします。Set this Promise to window.Promise so that transiled async functions will work on Firefox, Safari and IE, as well as with Zonejs and angular.
...
function enqueueNativeMicroTask(job) {
    ...
    nativePromiseThen.call(resolvedNativePromise, job); // ただ非同期でjobを実行しているだけ？
}
